...
import { ChatContext } from '../../context/GeneralContext';
...
const ProjectData = () => {
  ...
  const fetchProject = async(id) =>{
    await axios.get(`http://localhost:6002/fetch-project/${id}`).then(
      (response)=>{
    ...
    const handleBidding = async() =>{
      ...
      await axios.post("http://localhost:6002/make-bid", {clientId, freelancerId, projectId, proposal, bidAmount, estimatedTime}).then(
        (response)=>{
            setProposal('');
            setBidAmount(0);
            setEstimatedTime(0);
    ...
    const handleProjectSubmission = async() =>{
      ...
      await axios.post("http://localhost:6002/submit-project", {clientId, freelancerId, projectId, projectLink, manualLink, submissionDescription}).then(
        (response)=>{
            setProjectLink('');
            setManualLink('');
    ...
    const fetchChats = async() =>{
      await axios.get(`http://localhost:6002/fetch-chats/${params['id']}`).then(
        (response) =>{
    ...
    useEffect(() => {
      if (!socket) return;
      const handleBidApproved = ({ projectId }) => {
        socket.emit("join-chat-room", {
          projectId,
          freelancerId: localStorage.getItem("userId")
        });
      };
      socket.on("bid-approved", handleBidApproved);
      return () => {
        socket.off("bid-approved", handleBidApproved);
      };
    }, [socket]);
      ...
      {project ?
        ...
        <div className="project-data-page">
            ...
            <div className="project-chat-container">
                ...
                {(project.freelancerId === localStorage.getItem('userId') || project.clientId === localStorage.getItem('userId')) ?
                ...
                :
                <i style={{color: '#938f8f'}} >Chat will be enabled if the project is assigned to you or you are the client!!</i>
                }
